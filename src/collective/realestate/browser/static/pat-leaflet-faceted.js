...
(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    factory($, patterns.Base, patterns, patterns.Parser, patterns.logger,
      patterns.Leaflet, patterns['leaflet-awesomemarkers'],
      patterns['leaflet-fullscreen'], patterns['leaflet-providers'],
  ...
  var log = logger.getLogger('pat-leaflet-faceted');
  ...
  return Base.extend({
    name: 'leaflet-faceted',
    trigger: '.pat-leaflet-faceted',
    map: undefined,
    ...
    init: function initUndefined() {
      ...
      // MAP INIT
      map = this.map = new L.Map(this.$el[0], {
      ...
      // Minimap
      if (options.minimap) {
        var minimap = new L.Control.MiniMap(L.tileLayer.provider(options.default_map_layer), {
          toggleDisplay: true,
          mapOptions: {
            sleep: false
          }
